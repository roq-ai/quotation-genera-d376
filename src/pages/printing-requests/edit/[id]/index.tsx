...
import { getPrintingRequestById, updatePrintingRequestById } from 'apiSdk/printing-requests';
...
function PrintingRequestEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<PrintingRequestInterface>(
    () => (id ? `/printing-requests/${id}` : null),
    () => getPrintingRequestById(id),
  );
  ...
  const handleSubmit = async (values: PrintingRequestInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePrintingRequestById(id, values);
      mutate(updated);
      resetForm();
