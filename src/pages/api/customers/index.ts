...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCustomer() {
    ...
    const body = { ...req.body };
    if (body?.printing_request?.length > 0) {
      const create_printing_request = body.printing_request;
      body.printing_request = {
        create: create_printing_request,
      };
    } else {
      delete body.printing_request;
    }
